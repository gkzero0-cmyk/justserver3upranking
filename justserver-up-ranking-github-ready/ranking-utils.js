(function (root, factory) {
  ...
  function normalizeSoopCountToken(value) {
...
function countLowSoopFavoriteApplicants(comments, limit = 500) {
  ...
  function installLowSoopFavoriteUi(root) {
  const doc = root?.document;
  ...
  function apply() {
    ...
    if (filterActive && dataReady && matching === 0) {
      ...
      if (existingEmpty) existingEmpty.textContent = 'SOOP 즐겨찾기 500명 이하 신청자가 없습니다.';
      else if (!emptyRow) {
  ...
  if (tbody && root.MutationObserver) new root.MutationObserver(scheduleApply).observe(tbody, { childList: true, subtree: true });
    ...
    root.fetch = async function (...args) {
      ...
      try {
        ...
        if (String(requestUrl).includes('/api/comments')) {
          response.clone().json().then(data => {
            if (Array.isArray(data?.comments)) setData(data.comments);
          }).catch(() => {});
        }
      } catch {}
      return response;
